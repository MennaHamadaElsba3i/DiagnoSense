...
const AddPatient = () => {
  ...
  const isStep1Valid = formData.fullName.trim() && formData.age && selectedGender;
  ...
  const handleFiles = (category, files) => {
    const validFiles = Array.from(files).filter(file => {
      const validTypes = category === 'radiology'
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      apiFormData.append('first_name', formData.fullName.split(' ')[0] || formData.fullName);
      apiFormData.append('last_name', formData.fullName.split(' ').slice(1).join(' ') || 'N/A');
      apiFormData.append('age', formData.age);
      apiFormData.append('gender', selectedGender);
      apiFormData.append('email', formData.phone ? `${formData.phone}@temp.com` : 'noemail@temp.com');
      ...
      apiFormData.append('chronic_diseases', selectedChronicDiseases.join(', ') || 'none');
    ...
    <div>
      ...
      <aside className="sidebar">
        <div className="sidebar-logo">
          <span className="logo-text"><img src="DiagnoSense Logo Without BG.png" alt="" /></span>
        </div>
      ...
      <nav className="top-navbar">
        <div className="search-container">
          ...
          <input type="text" className="search-input" placeholder="Search patient or report…" />
        </div>
        ...
        <div className="wizard-card">
          ...
          <div className="wizard-body">
            ...
            <div className={`step-content ${currentStep === 1 ? 'active' : ''}`}>
              ...
              <div className="form-grid">
                <div className="form-group full-width">
                  ...
                  <input type="text" className="form-input" id="fullName" placeholder="Enter patient's full name" value={formData.fullName} onChange={handleInputChange} />
                </div>

                <div className="form-group">
                  <label className="form-label required">Age</label>
                  <input type="number" className="form-input" id="age" placeholder="Enter age" min="0" max="150" value={formData.age} onChange={handleInputChange} />
                </div>

                <div className="form-group">
                  <label className="form-label required">Gender</label>
                  <div className="radio-group">
                    <div className={`radio-button ${selectedGender === 'male' ? 'selected' : ''}`} onClick={() => handleGenderSelect('male')}>Male</div>
                    <div className={`radio-button ${selectedGender === 'female' ? 'selected' : ''}`} onClick={() => handleGenderSelect('female')}>Female</div>
                  </div>
                </div>

                <div className="form-group">
                  <label className="form-label">Phone Number</label>
                  <input type="tel" className="form-input" id="phone" placeholder="+20 XXX XXX XXXX" value={formData.phone} onChange={handleInputChange} />
                ...
                <div className="form-group">
                  ...
                  <input type="text" className="form-input" id="nationalId" placeholder="Enter ID number" value={formData.nationalId} onChange={handleInputChange} />
                  <div className="helper-text">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
              ...
              <div className="wizard-actions">
                <div></div>
                <button className="btn btn-primary" disabled={!isStep1Valid} onClick={() => goToStep(2)}>
                  Next Step
                  <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className={`step-content ${currentStep === 2 ? 'active' : ''}`}>
              ...
              <div className="toggle-section medhist">
                <div className="tog-item">
                  <label className="toggle-label">Is the patient a smoker? <span style={{ color: '#FF5C5C' }}>*</span></label>
                  <div className="radio-group">
              ...
              <div className="toggle-section">
                ...
                <div className="toggle-pills">
                  {['diabetes', 'hypertension', 'asthma', 'heart', 'kidney', 'liver', 'thyroid', 'cancer'].map(disease => (
                    <div key={disease} className={`pill ${selectedChronicDiseases.includes(disease) ? 'selected' : ''}`} onClick={() => handleChronicDiseaseToggle(disease)}>
                      {disease.charAt(0).toUpperCase() + disease.slice(1).replace('heart', 'Heart Disease').replace('kidney', 'Kidney Disease').replace('liver', 'Liver Disease')}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="wizard-actions">
                ...
                <button className="btn btn-primary" disabled={!isStep2Valid} onClick={() => goToStep(3)}>
                  Next Step
                  <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
                  </svg>
            ...
            <div className={`step-content ${currentStep === 3 ? 'active' : ''}`}>
                  ...
                  <div key={category} className="upload-card">
                    <div className="upload-card-header">
                      <div className="upload-card-title">
                        ...
                        <h4>{category === 'lab' ? 'Lab Tests' : category === 'history' ? 'Medical History' : 'Radiology Reports'}</h4>
                      </div>
                    ...
                    <input type="file" className="file-input-hidden" id={`${category}Input`} multiple accept={category === 'radiology' ? '.pdf,.docx,.doc,.jpg,.jpeg,.png,.dcm' : '.pdf,.docx,.doc,.jpg,.jpeg,.png'} onChange={(e) => handleFileInputChange(category, e)} />
              ...
              <div className="wizard-actions">
                <button className="btn btn-secondary" onClick={() => goToStep(2)} disabled={isProcessing}>
                  <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
                  </svg>
                  Back
